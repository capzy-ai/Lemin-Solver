...
**Solve Lemin slider puzzles with computer vision.**

[![Solve cost](https://img.shields.io/badge/from-%240.001%20%2F%20solve-%23ff5d2a)](https://capzy.ai/pricing)
[![Speed](https://img.shields.io/badge/avg%20solve-~8-12%20seconds-%2322c55e)](https://capzy.ai/products/lemin)
...
[Live Demo](https://capzy.ai/products/lemin/demo) ·
...
Copy-pasteable examples for solving **Lemin Cropped Captcha** through the
...
Lemin Cropped Captcha (leminnow.com) is a jigsaw-style slider puzzle — a piece is cropped from an image and placed on a rail; the user drags it to the correct position. Used on e-commerce, gaming, and financial sites. Returns the `answer` + `challengeId` pair the site validates server-side.

## Why Capzy

- **From $0.001 per solve.** Flat pricing — no tiers, no retainer, no monthly minimum.
- **~8-12 seconds average solve.** Production-grade speed.
...
|-----------|-------------|-------------:|
| `LeminTaskProxyLess`             | Proxyless (Capzy supplies the IP) | **$0.001**   |
| `LeminTask`                       | You supply the proxy              | **$0.001**   |
...
|-------|------|:--------:|-------|
| `type` | `string` | yes | LeminTaskProxyLess or LeminTask |
| `websiteURL` | `string` | yes | Full URL of the page where Lemin loads |
| `captchaId` | `string` | yes | Lemin captcha ID (starts with `CROPPED_`) |
| `proxyType` | `string` | no  | http | https | socks4 | socks5 (only for `LeminTask`) |
| `proxyAddress` | `string` | no  | IP or hostname of your proxy (only for `LeminTask`) |
| `proxyPort` | `integer` | no  | Port number of your proxy (only for `LeminTask`) |
| `proxyLogin` | `string` | no  | Optional — omit if your proxy doesn't require auth (only for `LeminTask`) |
| `proxyPassword` | `string` | no  | Optional — omit if your proxy doesn't require auth (only for `LeminTask`) |
...
- OpenCV gap detection
- Human-like drag simulation
- Works on every Lemin cropped variant

## FAQ

**Where do I find captchaId?** On the `#lemin-cropped-captcha` div's `data-captcha-id` attribute, or extract from the script URL: `https://api.leminnow.com/captcha/v1/cropped/{CAPTCHA_ID}/js`
